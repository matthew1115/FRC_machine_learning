...
def extract_elim_match_urls(url):
    """
    从指定URL提取elim-match-table中以"Quals"开头的<a>标签的URL
    返回URL列表
    """
    ...
    try:
        ...
        # 查找所有id为"elim-match-table"的表格
        elim_tables = soup.find_all('table', id='elim-match-table')
        
        if not elim_tables:
            print("未找到id为'elim-match-table'的表格")
            return
        
        match_urls = []
        
        # 遍历每个淘汰赛表格
        for table in elim_tables:
            # 查找表格中所有class为"visible-lg"的tr标签
            rows = table.find_all('tr', class_='visible-lg')
            
            for row in rows:
                # 在每行中查找<a>标签
                a_tags = row.find_all('a')
                for a_tag in a_tags:
                    # 检查<a>标签的文本是否以"Match"或"Final"开头
                    text = a_tag.text.strip()
                    if text and (text.startswith('Match') or text.startswith('Final')):
                        # 获取href属性并转换为绝对URL
                        href = a_tag.get('href')
                        if href:
                            absolute_url = urljoin(url, href)
                            match_urls.append(absolute_url)
        
        # 打印所有找到的URL
        print(f"找到 {len(match_urls)} 个Match/Final开头的链接")
        return match_urls
...
def main():
    ...
    # 提取资格赛链接
    url = "https://www.thebluealliance.com/event/2025xxsan"
    print(f"event_url:{url}")
    elim_match_url = extract_elim_match_urls(url)
    if elim_match_url:
        qual_links.extend(elim_match_url)
    ...
    if qual_links:
        ...
        filename = f"FRC2025_PLAYOFF_SY_{timestamp}.csv"
        ...
        try:
            with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                # 测试单场数据
				# qual_links = []
                # qual_links.append("https://www.thebluealliance.com/match/2025cnsh_sf4m1")
                for qm_url in qual_links:
                    print(f"deail page url:{qm_url}")
                    # 提取表格数据
