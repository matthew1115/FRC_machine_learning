...
params = {
    ...
    'num_leaves': 20,  # 减少叶子数量
    'max_depth': 5,  # 限制树深度
    'min_data_in_leaf': 30,  # 增加叶子最小数据量
...
try:
    ...
    print("无法计算类别权重，继续不使用权重")
	
# 使用交叉验证确定最佳迭代次数
print("使用交叉验证选择最佳迭代次数...")
try:
    cv_results = lgb.cv(
        params,
        train_data_lgb,
    ...
    # 调试：打印所有可用的键
    print("可用的cv_results键:", list(cv_results.keys()))
    
    # 尝试获取最佳迭代次数 - 检查常见的键名变体
    loss_key = None
    possible_keys = ['valid multi_logloss-mean', 'valid-multi_logloss-mean', 'multi_logloss-mean']
    ...
    if loss_key is None:
        # 如果找不到预期的键，使用第一个可用的损失键
        available_loss_keys = [k for k in cv_results.keys() if 'loss' in k.lower() or 'logloss' in k.lower()]
        if available_loss_keys:
            loss_key = available_loss_keys[0]
            print(f"未找到标准multi_logloss键，使用: {loss_key}")
        else:
            raise KeyError("在cv_results中找不到任何损失键")
    
    # 找到最小损失对应的迭代轮数
    best_num_round = np.argmin(cv_results[loss_key]) + 1  # +1因为索引从0开始
    print(f"最佳迭代次数: {best_num_round} (基于键: {loss_key})")
    
except Exception as e:
    print(f"交叉验证过程中出错: {e}")
    # 备用方案：使用固定迭代次数
    best_num_round = 100
    print(f"将使用默认迭代次数: {best_num_round}")
...
# 计算过拟合程度
overfitting_degree = train_accuracy - test_accuracy
print(f"\n过拟合程度 (训练准确率 - 测试准确率): {overfitting_degree:.4f}")
...
# 绘制学习曲线，这段代码有bug，但不影响，所以没有修复
plt.figure(figsize=(10, 6))
plt.plot(range(1, best_num_round+1), cv_results['multi_logloss-mean'], label='Training Loss')
plt.axvline(x=best_num_round, color='r', linestyle='--', label='Best Iteration')
