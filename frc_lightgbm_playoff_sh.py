...
params = {
    ...
    'metric': 'multi_logloss',
    'boosting_type': 'gbdt',
    'learning_rate': 0.01,  # 降低学习率。原0.01
    'num_leaves': 20,  # 过拟合则减少叶子数量。 小于 2^(max_depth), 31-300。原：20	
    'max_depth': 5,  # 过拟合则限制树深度。原：5
    'min_data_in_leaf': 30,  # 过拟合则增加叶子最小数据量。原：30
    'min_split_gain': 0.01,  # 过拟合则增加分裂最小增益。原：0.01
    'feature_fraction': 0.7,  # 过拟合则减少特征采样比例。原：0.7
    'bagging_fraction': 0.7,  # 过拟合则减少数据采样比例。原：0.7
    'bagging_freq': 5,
    'lambda_l1': 0.1,  # 过拟合则增加L1正则化。原：0.1
    'lambda_l2': 0.1,  # 过拟合则增加L2正则化。原：0.1
    'min_child_samples': 20,  # 过拟合则增加叶子最小样本数。原：20
    'verbose': -1,
    'random_state': 42,
    'n_estimators': 200,      # 弱学习器（树）的数量
...
def plot_auc_curves(y_true, y_pred, title):
    ...
    # 计算宏观AUC
    all_fpr = np.unique(np.concatenate([fpr for i in range(2)]))
