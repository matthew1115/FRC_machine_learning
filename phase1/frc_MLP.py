...
class MLP(nn.Module):
    def __init__(self, input_dim, hidden_dims, output_dim, dropout_rate=0.3):
        ...
        for i, hidden_dim in enumerate(hidden_dims):
            ...
            # 只在中间层添加BatchNorm和Dropout
            if i < len(hidden_dims) - 1:  # 不在最后一层隐藏层添加
...
def main():
    ...
    # 初始化MLP模型 - 简化版本以减少过拟合
    input_dim = 6 * 14  # 6支队伍 * 14个特征
    hidden_dims = [128, 64]  # 减少隐藏层维度和数量
    output_dim = len(train_dataset.label_encoder.classes_)
    dropout_rate = 0.3  # 增加dropout率
    
    print(f"MLP配置: 输入维度={input_dim}, 隐藏层={hidden_dims}, 输出维度={output_dim}")
    
    model = MLP(input_dim, hidden_dims, output_dim, dropout_rate).to(device)
    
    # 定义损失函数和优化器
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=0.001, weight_decay=1e-4)  # 增加权重衰减
