...
# 准备训练数据 - 使用实际特征值
def prepare_train_data(matches, feature_columns):
    features = []
    labels = []
    ...
    for match in matches:
        ...
        for _, row in match.iterrows():
            # 使用实际特征值
            raw_features = row[feature_columns].values.astype(float)
...
# 准备测试数据 - 使用历史平均特征值
def prepare_test_data(matches, team_stats, feature_columns):
    features = []
    labels = []
    ...
    for match in matches:
        ...
        for _, row in match.iterrows():
            ...
            if team_id in team_stats:
                ...
                # 如果队伍没有历史数据，使用零向量
                match_features.extend(np.zeros(len(feature_columns)))
...
def main():
    ...
    # 准备训练数据 - 使用实际特征值
    X_train, y_train = prepare_train_data(train_matches, feature_columns)
    
    # 准备测试数据 - 使用历史平均特征值
    X_test, y_test = prepare_test_data(test_matches, team_stats, feature_columns)
    ...
    for i in range(6):  # 6支队伍
        for j in range(len(feature_columns)):  # 特征数量
            feature_names.append(f"Team_{i+1}_Feature_{j+1}")
    ...
    joblib.dump(team_stats, 'team_stats.pkl')  # 保存队伍统计信息
    print("模型已保存")
    ...
    # 模拟真实预测场景
    print("\n模拟真实预测场景:")
    print("在真实预测中，我们只能使用历史平均特征值进行预测")
    print("训练时使用实际特征值，预测时使用历史平均特征值")
    print("这种差异可能会导致性能下降")
